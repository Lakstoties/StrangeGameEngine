...
namespace SGE::Sound
{
    ...
    float Channel::ArpeggioEffect::Apply(float sourceOffsetIncrement)
    {
        ...
        if (Enabled)
        {
            ...
            //  Based on that state alter the arpeggio offset increment
            if (State == 1)
            {
                //  If at state 1, calculate the increment for X semitones
                sourceOffsetIncrement = float(sourceOffsetIncrement * pow(Precalculated::SEMITONE_MULTIPLIER, SemitoneX));
            }

            else if (State == 2)
            {
                //  If at state 2, calculate the increment for Y semitones
                sourceOffsetIncrement = float(sourceOffsetIncrement * pow(Precalculated::SEMITONE_MULTIPLIER, SemitoneY));
            }
    ...
    void Channel::VibratoEffect::Set(float amplitude, float cycles)
    {
        ...
        //  Set the cycle rate for the Vibrato so that (X * Ticks) / 64 cycles occur in the division
        //  If 0, use previous settings
        if (cycles != 0)
        {
            Cycles = cycles;
    ...
    float Channel::VibratoEffect::Apply(float sourceOffsetIncrement)
    {
        if (Enabled)
        {

            //Calculate the offset
            sourceOffsetIncrement = sourceOffsetIncrement + (Amplitude * waveform[currentWaveformPosition]);

            //Increment the vibrato waveform position
            currentWaveformPosition = (int)((float)currentWaveformPosition + Cycles) % SAMPLE_RATE;
        }
    ...
    float Channel::PeriodSlideEffect::Apply(float sourceOffsetIncrement)
    {
        if (Enabled)
        {
            ...
            if (CurrentSamples >= SampleInterval)
            {
                ...
                float offsetPeriod = PeriodBasis / (SGE::Sound::SAMPLE_RATE * sourceOffsetIncrement);
                ...
                sourceOffsetIncrement = PeriodBasis / (offsetPeriod * SGE::Sound::SAMPLE_RATE);
    ...
    float Channel::RetriggerEffect::Apply(float sourceOffset)
    {
        ...
        if (Enabled)
        {
            ...
            if (CurrentSamples >= SampleInterval)
            {
                //  Set the sample sourceOffset back to the designated destination
                sourceOffset = (float)SampleDestination;
    ...
    void Channel::Render(unsigned int numberOfSamples, renderSampleType* sampleBuffer)
    {
        ...
        for (unsigned int i = 0; i < numberOfSamples; i++)
        {
            ...
            else
            {
                ...
                sampleBuffer[i] = (renderSampleType)((float)SGE::Sound::SampleBuffers[CurrentSampleBuffer][(unsigned int)offset] * Volume);

                //  Add to the accumulator for the average
    ...
    //
    //
    // Sound Sample Buffer Definitions
    //
    //

    //
    //  Overlord boolean operator
    //
    ...
    {
        //
        // Check for legitimate number of samples
        //
        if (numOfSamples < 1)
        {
            //Info the error
    ...
    namespace Waveforms
    {
        ...
        void PreGenerateSine()
        {
            ...
            for (unsigned int i = 0; i < SAMPLE_RATE; i++)
            {
                ...
                Sine[i] = sinf(periodStepping * (float)i);
            }
        }
        ...
        void PreGenerateRampDown()
        {
            ...
            for (unsigned int i = 0; i < SAMPLE_RATE; i++)
            {
                ...
                RampDown[i] = 1.0f - ((float)i / (float)((float)SAMPLE_RATE / 2));
            }
        }
        ...
        void PreGenerateSquare()
        {
            ...
            {
                //Put all the data in the Square waveform array.
                Square[i] = 1.0f;
            }
    ...
    //
    //Variables to hold average volume levels from the last callback from PortAudio
    //
    ...
    void Start()
    {
        ...
        {
            SGE::System::Message::Output(SGE::System::Message::Levels::Information, SGE::System::Message::Sources::Sound, std::to_string(audioSystem.getDeviceCount()) + " Audio Devices detected.\n");
        }
        ...
        try
        {
            audioSystem.openStream(&audioStreamParameters, nullptr, RTAUDIO_SINT16, SGE::Sound::SAMPLE_RATE, &RTAUDIO_RENDER_BUFFER_SIZE, &AudioCallback, nullptr, &audioStreamOptions);
        ...
        catch (RtAudioError & e)
        {
            ...
            SGE::System::Message::Output(SGE::System::Message::Levels::Error, SGE::System::Message::Sources::Sound, "Error Output From RtAudio Exception: " + e.getMessage() + "\n");
            e.printMessage();
        }
