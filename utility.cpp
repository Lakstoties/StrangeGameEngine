		...
		namespace Timer
        {
			...
			float TimerDelta::Stop()
			{
				if (started)
				{
					...
					//  Return the delta in
					//
					//  Summary:  Calculated the time delta, cast it down to milliseconds, and get the count of milliseconds, then multiply the rate, and dividie by a 1000 milliseconds in a second
					//
			...
			void ModulePlayer::PlayThread()
			{
				...
				while (PlayerThreadActive)
				{
					...
					while (!Location.EndOfModule && PlayerThreadActive)
					{
						...
						//  Set up the channels
						//
						SGE::System::Message::Output(SGE::System::Message::Levels::Debug, SGE::System::Message::Sources::Utility, "Mod Player: " + std::string(modFile.title) + " - Pat: " + std::to_string(Location.Position) + " - Div: " + std::to_string(Location.Division) + " - Time: " + std::to_string(deltaTime) + "\n");
