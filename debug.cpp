...
namespace SGE::Debug
{
    ...
    //
    void DrawSampleBuffersStatus(int xCornerPosition, int yCornerPosition)
    {
        ...
        for (auto & Channel : SGE::Sound::Channels)
        {
            if (Channel.CurrentSampleBufferSet)
            {
                SGE::Render::DrawRectangle(
                    xCornerPosition + SIDE_SPACING - 1 + (Channel.CurrentSampleBuffer % 64) * STATUS_SQUARE_X_SPACING,
                    yCornerPosition + TOP_SPACING - 1 + (Channel.CurrentSampleBuffer / 64) * STATUS_SQUARE_Y_SPACING,
                    STATUS_SQUARE_SIZE + 2,
                    STATUS_SQUARE_SIZE + 2,
                    Channel.Playing ? SGE::Render::Colors::ColorMode8Bit[255] : SGE::Render::Colors::ColorMode8Bit[0]
                );
            }
        }
    }
