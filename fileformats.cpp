...
namespace SGE
{
	namespace FileFormats
	{
		...
		int ModuleFile::LoadFile(std::string targetFilename)
		{
			...
			{
				//Read sample title
				moduleFile.read((char*)&samples[i].title, 22);
                totalReadCount += moduleFile.gcount();
				...
				moduleFile.read((char *)&samples[i].lengthInWords, 2);
                totalReadCount += moduleFile.gcount();
				...
				moduleFile.read((char *)&samples[i].finetune, 1);
                totalReadCount += moduleFile.gcount();
				...
				moduleFile.read((char *)&samples[i].volume, 1);
                totalReadCount += moduleFile.gcount();
				...
				moduleFile.read((char *)&samples[i].repeatOffset, 2);
                totalReadCount += moduleFile.gcount();
				...
				moduleFile.read((char *)&samples[i].repeatLength, 2);
                totalReadCount += moduleFile.gcount();
			...
			for (int i = 0; i < 31; i++)
			{
				if (samples[i].lengthInWords > 1)
				{
					...
					moduleFile.read((char*)samples[i].data, samples[i].SampleLengthInBytes());
                    totalReadCount += moduleFile.gcount();

					//Check to see if we actually read enough bytes
					if (moduleFile.gcount() != samples[i].SampleLengthInBytes())
					{
						//This file is way to small to be a proper wav file
						SGE::System::Message::Output(SGE::System::Message::Levels::Error, SGE::System::Message::Sources::FileFormats, "Sound System Module File \"" + targetFilename + "\" is not correct format - File Too Small to be proper.  Sample Data Reported: " + std::to_string(samples[i].lengthInWords * 2) + "  Sample Data Read: " + std::to_string(moduleFile.gcount()) + " \n");
						//return -13;
					}
				}
			}
